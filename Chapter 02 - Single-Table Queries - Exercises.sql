...
SELECT orderid
	 ...
	 , empid
FROM Sales.Orders
WHERE orderdate = EOMONTH(orderdate);  --> EO MONTH Function is a quick abstraction to get the end of the month date. 
...
-- The first query filters the table using the where clause, the second query filters the empid groups using the Having clause on the latest order dates. 
-- Query 1 returns the orders that the employee had prior to May 1st, 2016 
-- Query 2 returns the employees that had orders prior to May 1st, 2016 and the number of those orders. 
...
SELECT custid
	 , orderdate
	 , orderid
	 , ROW_NUMBER() OVER(PARTITION BY custid ORDER BY orderdate, orderid) AS rownum -- First time using ROW_NUMBER and OVER(PARTITION BY)
FROM Sales.Orders; 
